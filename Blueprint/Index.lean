...
It's not clear to me if we should be working with additive groups throughout the
project. There are two places where they seem necessary:
...
1. If we want to put a topology (or more broadly, an uniform structure)
   ...
   uniformity.
2. {bpref "multisubadditivemap"}[]

In case we need an {InlineLean.name}`AddGroup` for the main definition, we will
also need an `ESeminormedAddGroup` class for stating the results in `ELorentz`,
because they depend on the class `ESeminormedAddMonoid`.

## Verso-blueprint

`verso-blueprint` is developed using AI and does not work as well as I would
like it to in some aspects. The web style of its components is not very formal
and it does not emit proper $`\LaTeX` for the theorems and similar constructs.
I chose to make some patches to its style in the module
`Blueprint.StylePatches`, also using AI.
